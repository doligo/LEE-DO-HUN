...
void chess_pieces::Init_pieces_2(HDC hdc, int player_num)
{
	if (player_num == 0)
	{
		if (m_King.draw == FALSE)
		{
			...
			hbtmap = (HBITMAP)LoadImage(NULL, "C:\\sos\\LEE-DO-HUN\\참고코드\\과제\\WinApi과제\\챕터7\\체스게임\\block_w_05.bmp", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION | LR_DEFAULTSIZE | LR_LOADFROMFILE);
		...
		if (m_Queen.draw == FALSE)
		{
			...
			hbtmap = (HBITMAP)LoadImage(NULL, "C:\\sos\\LEE-DO-HUN\\참고코드\\과제\\WinApi과제\\챕터7\\체스게임\\block_w_04.bmp", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION | LR_DEFAULTSIZE | LR_LOADFROMFILE);
		...
		for (int i = 0; i < 2; i++)
		{
			if (m_Rook[i].draw == FALSE)
			{
				...
				hbtmap = (HBITMAP)LoadImage(NULL, "C:\\sos\\LEE-DO-HUN\\참고코드\\과제\\WinApi과제\\챕터7\\체스게임\\block_w_03.bmp", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION | LR_DEFAULTSIZE | LR_LOADFROMFILE);
		...
		for (int i = 0; i < 2; i++)
		{
			if (m_Bishop[i].draw == FALSE)
			{
				...
				hbtmap = (HBITMAP)LoadImage(NULL, "C:\\sos\\LEE-DO-HUN\\참고코드\\과제\\WinApi과제\\챕터7\\체스게임\\block_w_02.bmp", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION | LR_DEFAULTSIZE | LR_LOADFROMFILE);
		...
		for (int i = 0; i < 2; i++)
		{
			if (m_Knight[i].draw == FALSE)
			{
				...
				hbtmap = (HBITMAP)LoadImage(NULL, "C:\\sos\\LEE-DO-HUN\\참고코드\\과제\\WinApi과제\\챕터7\\체스게임\\block_w_01.bmp", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION | LR_DEFAULTSIZE | LR_LOADFROMFILE);
		...
		for (int i = 0; i < 8; i++)
		{
			if (m_Pawn[i].draw == FALSE)
			{
				...
				hbtmap = (HBITMAP)LoadImage(NULL, "C:\\sos\\LEE-DO-HUN\\참고코드\\과제\\WinApi과제\\챕터7\\체스게임\\block_w_00.bmp", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION | LR_DEFAULTSIZE | LR_LOADFROMFILE);
	...
	else if (player_num == 1)
	{
		...
		{
			MemDC[0] = CreateCompatibleDC(hdc);
			hbtmap = (HBITMAP)LoadImage(NULL, "C:\\sos\\LEE-DO-HUN\\참고코드\\과제\\WinApi과제\\챕터7\\체스게임\\block_b_05.bmp", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION | LR_DEFAULTSIZE | LR_LOADFROMFILE);
			old_hbtmap = (HBITMAP)SelectObject(MemDC[0], hbtmap);
			GetObject(hbtmap, sizeof(BITMAP), &btmap);
			m_x = btmap.bmWidth;
			m_y = btmap.bmHeight;
		...
		{
			MemDC[1] = CreateCompatibleDC(hdc);
			hbtmap = (HBITMAP)LoadImage(NULL, "C:\\sos\\LEE-DO-HUN\\참고코드\\과제\\WinApi과제\\챕터7\\체스게임\\block_b_04.bmp", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION | LR_DEFAULTSIZE | LR_LOADFROMFILE);
			old_hbtmap = (HBITMAP)SelectObject(MemDC[1], hbtmap);
			GetObject(hbtmap, sizeof(BITMAP), &btmap);
			m_x = btmap.bmWidth;
			m_y = btmap.bmHeight;
		...
		for (int i = 0; i < 2; i++)
		{
			...
			{
				MemDC[i + 2] = CreateCompatibleDC(hdc);
				hbtmap = (HBITMAP)LoadImage(NULL, "C:\\sos\\LEE-DO-HUN\\참고코드\\과제\\WinApi과제\\챕터7\\체스게임\\block_b_03.bmp", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION | LR_DEFAULTSIZE | LR_LOADFROMFILE);
				old_hbtmap = (HBITMAP)SelectObject(MemDC[i + 2], hbtmap);
				GetObject(hbtmap, sizeof(BITMAP), &btmap);
				m_x = btmap.bmWidth;
				m_y = btmap.bmHeight;
		...
		for (int i = 0; i < 2; i++)
		{
			...
			{
				MemDC[i + 4] = CreateCompatibleDC(hdc);
				hbtmap = (HBITMAP)LoadImage(NULL, "C:\\sos\\LEE-DO-HUN\\참고코드\\과제\\WinApi과제\\챕터7\\체스게임\\block_b_02.bmp", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION | LR_DEFAULTSIZE | LR_LOADFROMFILE);
				old_hbtmap = (HBITMAP)SelectObject(MemDC[i + 4], hbtmap);
				GetObject(hbtmap, sizeof(BITMAP), &btmap);
				m_x = btmap.bmWidth;
				m_y = btmap.bmHeight;
		...
		for (int i = 0; i < 2; i++)
		{
			...
			{
				MemDC[i + 6] = CreateCompatibleDC(hdc);
				hbtmap = (HBITMAP)LoadImage(NULL, "C:\\sos\\LEE-DO-HUN\\참고코드\\과제\\WinApi과제\\챕터7\\체스게임\\block_b_01.bmp", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION | LR_DEFAULTSIZE | LR_LOADFROMFILE);
				old_hbtmap = (HBITMAP)SelectObject(MemDC[i + 6], hbtmap);
				GetObject(hbtmap, sizeof(BITMAP), &btmap);
				m_x = btmap.bmWidth;
				m_y = btmap.bmHeight;
		...
		for (int i = 0; i < 8; i++)
		{
			...
			{
				MemDC[i + 8] = CreateCompatibleDC(hdc);
				hbtmap = (HBITMAP)LoadImage(NULL, "C:\\sos\\LEE-DO-HUN\\참고코드\\과제\\WinApi과제\\챕터7\\체스게임\\block_b_00.bmp", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION | LR_DEFAULTSIZE | LR_LOADFROMFILE);
				old_hbtmap = (HBITMAP)SelectObject(MemDC[i + 8], hbtmap);
				GetObject(hbtmap, sizeof(BITMAP), &btmap);
				m_x = btmap.bmWidth;
				m_y = btmap.bmHeight;
